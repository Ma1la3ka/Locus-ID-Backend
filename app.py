...
# ── MediaPipe face detector (lazy singleton) ──────────────────
...
def detect_face(image_path: str) -> bool:
    """Returns True if at least one face found — uses MediaPipe only."""
...
def compare_faces(stored_path: str, live_path: str):

    def face_histogram(path):
        img = cv2.imread(path)
        if img is None:
            return None
        rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    ...
    h1 = face_histogram(stored_path)
    h2 = face_histogram(live_path)
...
def register_user():
    ...
    try:
        ...
        with db.cursor() as c:
            ...
            if role == "Student":
                c.execute(f"INSERT INTO {table} ({id_col},first_name,last_name,password,faculty,department,face_encoding,photo_path) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)",
                          (user_id,f_name,l_name,hashed,faculty,dept,"[]","no_photo_yet.jpg"))
            else:
                c.execute(f"INSERT INTO {table} ({id_col},first_name,last_name,password,face_encoding,photo_path) VALUES(%s,%s,%s,%s,%s,%s)",
...
def register_with_face():
    """
    Multipart: face (JPEG) + userId/firstName/lastName/password/faculty/department
    Uses MediaPipe ONLY for face detection — no DeepFace, no TensorFlow.
    """
    if request.method == "OPTIONS": return jsonify({"status": "ok"}), 200
    db = None
    photo_path = None
    try:
        ...
        # MediaPipe face check — lightweight, no TensorFlow
        if not detect_face(photo_path):
...
def verify_face_attendance():
    """
    GPS re-check + MediaPipe histogram face comparison.
    No DeepFace, no TensorFlow.
    """
    if request.method == "OPTIONS": return jsonify({"status": "ok"}), 200
    ...
    temp_path = os.path.join(FACE_PHOTOS_DIR, f"temp_{uid}.jpg")
    ...
    try:
        ...
        with db.cursor(pymysql.cursors.DictCursor) as c:
            ...
            # 4. MediaPipe histogram comparison — no TensorFlow
            try:
                matched, score = compare_faces(registered_path, temp_path)
                print(f"[ATTEND] {uid} | matched={matched} | score={score:.1f}%")
                if not matched:
                    return jsonify({
                        "status":  "face_mismatch",
                        "message": f"Face does not match ({score:.1f}% similarity). Try again in better lighting."
                    })
    ...
    try:
        with db.cursor(pymysql.cursors.DictCursor) as c:
            ...
            for s in sessions:
                ...
                log_data.append({"date":s["created_at"].strftime("%Y-%m-%d"),"course":f"{s['course_code']}: {s['course_name']}","status":"Ongoing" if s["status"]=="Active" else "Ended","duration":dur})
    ...
    try:
        with db.cursor(pymysql.cursors.DictCursor) as c:
            ...
            for s in sessions:
                ...
                log.append({"date":s["created_at"].strftime("%Y-%m-%d") if s["created_at"] else "N/A","status":"Present" if present else "Absent","time":records[s["id"]].strftime("%I:%M %p") if present and records.get(s["id"]) else "--:--"})
            total   = len(sessions)
        ...
        with db.cursor(pymysql.cursors.DictCursor) as c:
            c.execute("""SELECT CONCAT(s.first_name,' ',s.last_name) AS full_name,s.student_id,s.faculty,s.department,
                (SELECT IFNULL((COUNT(a.id)*100.0/NULLIF((SELECT COUNT(id) FROM attendance_sessions WHERE course_code=%s),0)),0)
                 FROM attendance_records a JOIN attendance_sessions sess ON a.session_id=sess.id WHERE a.student_id=s.student_id AND sess.course_code=%s) AS attendance_rating
                FROM students s JOIN student_enrollments e ON s.student_id=e.student_id WHERE e.course_code=%s""",(course_code,course_code,course_code))
    ...
    try:
        with db.cursor(pymysql.cursors.DictCursor) as c:
            c.execute("""SELECT CONCAT(s.first_name,' ',s.last_name) AS full_name,a.student_id,a.timestamp AS marked_at
                FROM attendance_records a JOIN students s ON a.student_id=s.student_id WHERE a.session_id=%s ORDER BY s.first_name ASC""",(session_id,))
            records = c.fetchall()
            ...
            for r in records:
                writer.writerow([r["full_name"],r["student_id"],"Present",r["marked_at"].strftime("%H:%M:%S") if r["marked_at"] else "N/A"])
            output.seek(0)
...
def get_notifications():
    ...
    try:
        with db.cursor(pymysql.cursors.DictCursor) as c:
            c.execute("SELECT id,course_code,message,created_at FROM announcements WHERE course_code IN (SELECT course_code FROM student_enrollments WHERE student_id=%s) ORDER BY created_at DESC",(uid,))
            notifs = c.fetchall()
